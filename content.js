...
async function createBookmarkButton(referenceNode) {
  const bookmarkButton = document.createElement("button");
  bookmarkButton.className =
    "bookmark-icon gizmo:pl-0 rounded-md disabled:dark:hover:text-gray-400 dark:hover:text-gray-200 dark:text-gray-400 text-gray-400 hover:text-gray-950 md:invisible md:group-hover:visible md:group-[.final-completion]:visible";
  ...
  const testId = ancestorWithTestId
    ...
    : null;
  var bookmarked = await isBookmarked(testId);
...
async function addBookmarkButton(div) {
  if (div.querySelector('img[alt="Generated by DALL·E"]')) {
    return;
  }

  const specificChild = div.querySelector(
    "div > div > div.relative.flex.w-full.flex-col.lg\\:w-\\[calc\\(100\\%-115px\\)\\].agent-turn > div.flex-col.gap-1.md\\:gap-3 > div.mt-1.flex.justify-start.gap-3.empty\\:hidden"
  );

  if (specificChild && !specificChild.querySelector(".bookmark-icon")) {
    specificChild.classList.remove("justify-start");
    specificChild.classList.add("justify-between");
    const bookmarkButton = await createBookmarkButton(div);
    specificChild.appendChild(bookmarkButton);
  }
}

async function handleBookmarkButtonMutation(mutations) {
...
const bookmarkButtonObserver = new MutationObserver(async (mutations) => {
